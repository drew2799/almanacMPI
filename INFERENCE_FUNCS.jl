    ...
    function AlmPrior_PB(ȳ)
        ...
        if MPI.Comm_rank(comm) == root
            ...
            for l in 1:lmax+1
                ...
                adj_sample_Kl[l] = ((l-0.5)/sample_Cl[l] - Al/(sample_Cl[l]^2))*1_500*pdf(Normal(0,1), sample_Kl[l])*ȳ
            end
        else
            adj_sample_alm = nothing
            adj_sample_Kl = nothing
        end
...
function NegLogPosterior(θ, θ_length, comm, ncore, helper_DMap; data = gen_DMap, lmax=lmax, nside=nside, invN = invN_DMap, root = 0)
